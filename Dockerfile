...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get -y --no-install-recommends install tzdata apt-utils wget git bzip2 make cmake xsltproc gcc g++ pkg-config zlib1g-dev libxml2-dev python3 python3-distutils default-jre
...
RUN rm -rf /var/lib/apt/lists/* && apt-get purge -y wget git bzip2 make cmake xsltproc gcc g++ pkg-config && apt-get clean
